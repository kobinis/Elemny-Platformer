...
{
    [Serializable]
    public class MoveToTarget : BaseUpdate
    {
        ...
        public MoveToTarget(ProjectileTargetType target, float force, float maxSpeed, bool speedWarping = false)
        {
            this.Target = target;
        ...
        public override void Update(Projectile projectile, float normalizedLifeTime, GameEngine gameEngine)
        {
            ...
            if (target != null)
            {
                if (Force != 0)
                {
                    Vector2 dif = target.Position - projectile.Position;
                    ...
                    if (dif != Vector2.Zero)
                    {
                        ...
                        else
                        {
                            ...
                            if (SpeedWarping)
                            {
                                ...
                                maxSpeed = Math.Max( maxSpeed * distance * 0.05f,MaxSpeed); // TODO: take all the 3 magic number and create variables from them.
