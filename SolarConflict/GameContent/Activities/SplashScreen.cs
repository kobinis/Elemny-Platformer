...
{    
    public class SplashScreen:Activity
    {
        ...
        private GuiControl MakeGui()
        {
            ...
            control.ControlColor = Color.DarkGray;
            string title;
            if (!_content.TryGetValue("title", out title))
                title = "Level Summary";
            var titleControl = new RichTextControl(title, Game1.menuFont);
            titleControl.Parser.IsTextBorder = true;
            control.AddChild(titleControl);
            //Image
            foreach (var line in _lines)
            {
                RichTextControl lineControl = new RichTextControl(line);
                lineControl.Parser.IsTextBorder = true;
                lineControl.MaxLineWidth = ActivityManager.ScreenWidth * 0.95f;
